...
class ExportarDesembolsos implements   FromCollection, WithHeadings, ShouldAutoSize, WithEvents,WithDrawings
{
    ...
    */
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                 $event->sheet->getDelegate()->getStyle('A1:T4')->getFont()->getColor()
                ...
                $event->sheet->getDelegate()->setAutoFilter('A4:T4');
                $event->sheet->getStyle('A4:T4')->applyFromArray([
                    'borders' => [
                        'outline' => [
                            'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN,
                            'color' => ['argb' => 'ff000000'],
                ...
                $event->sheet->getDelegate()->getStyle('A4:T4')->getFont()->setSize(12);
                $event->sheet->getDelegate()->getStyle('A4:T4')->getFont()->setBold(true);
                ...
                //CONTADOR
                $contador = 4;
                $varlista =  $this->exportarReporte_desembolsos($this->fechaIni, $this->fechaFin);
                foreach ($varlista as $item){
                    $contador = $contador + 1;
                }
                $celdas = "A4:T".$contador;
    ...
    {
        $exportarReporte_desembolsos=  $this->exportarReporte_desembolsos($this->fechaIni, $this->fechaFin);
        return $exportarReporte_desembolsos;
    }
    ...
    {
        $contador = 0;
        $varlista =  $this->obtenerReporte_desembolsos($this->fechaIni, $this->fechaFin);
        foreach ($varlista as $item){
            $contador = $contador + 1;
        }
        ...
        return [
            ...
            [
                ...
                'COBERTURA',
                'IVA COBERTURA',
                'TOTAL IMPORTE CARTERA',
                'TOTAL IMPORTE DESEMBOLSADO',
                'DISTRIBUIDOR',
                'TOTAL INTERESES Y COBERTURA',
                'TOTAL IVA INTERESES Y COBERTURA'
            ]
        ];
    }

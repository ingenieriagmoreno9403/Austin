...
<head>
    ...
    <title>Cotizacion de {{ $tipo }}</title>
...
<body>
    ...
    <div class="header">
        ...
        <div class="quotation-title"><b>COTIZACIÓN {{$folio}}</b></div>
    ...
    <div class="main-content">
            ...
            <tbody>
                <tr>
                    <td class="company-block">
                        <div>{{$item->nombre_atencion}}</div>
                        <div>{{$item->rfc_cliente}}</div>
                        <div>{{$item->direccion_cliente}}7</div>
                        <div>{{$item->telefono_atencion}}</div>
                        <div>{{$item->correo_atencion}}</div>
                    </td>
                    
                    <td class="company-block">
                        <div>{{$item->nombre_empresa_vendedor}}</div>
                        <div>{{$item->rfc_empresa_vendedor}}</div>
                        <div>{{$item->direccion_empresa_vendedor}}</div>
                        <div>{{$item->telefono_vendedor}}</div>
                        <div>{{$item->correo_vendedor}}</div>
                    </td>
                </tr>
                </tbody>
        </table>
        ...
        <div class="service-section">
            <div class="service-header">SERVICIO {{$tipo}}</div>
            @php($colspan = 0)
            <table class="table">
                @if(strtolower($tipo) == 'suministro')
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="description-cell2">{{$i++}}</td>
                            <td class="description-cell2">{{$item->cantidad}}</td>
                            <td class="description-cell">{{$item->descripcion_producto}}</td>
                            <td class="description-cell">{{$item->unidad_medida}}</td>
                            <td class="description-cell">{{$item->otrosconceptos1}}</td>
                ...
                @elseif(strtolower($tipo) == 'integracion' || strtolower($tipo) == 'integración')
                    ...
                    <tbody>
                        ...
                        @foreach($productos_suministro as $item)
                        <tr>
                            <td class="description-cell2">{{$item->partida}}</td>
                            <td class="description-cell2">{{$item->cantidad}}</td>
                            <td class="description-cell">{{$item->descripcion}}</td>
                            <td class="description-cell">{{$item->unidad}}</td>
                            <td class="text-truncate description-cell2">$ {{number_format($item->{'Precio u.'},2)}}</td>
                            <td class="description-cell">{{$item->observacion}}</td>
                            <td class="text-truncate description-cell2">$ {{number_format($item->total,2)}}</td>
                        </tr>
                        @php($suma_total = $suma_total + $item->total)
                        @endforeach
                ...
                @else
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="description-cell2">{{$i++}}</td>
                            <td class="description-cell2">{{$item->cantidad_total}}</td>
                            <td class="description-cell" colspan="2">{{$item->nombre}}</td>
                            <td class="description-cell">{{$item->unidad_medida}}</td>
                            <td class="text-truncate description-cell">$ {{number_format($item->precio_unitario,2)}}</td>
                            <td class="description-cell">{{$item->otrosconceptos1}}</td>
                            @php($total = $item->precio_unitario*$item->cantidad_total)
                            <td class="text-truncate description-cell2">$ {{number_format($total,2)}}</td>
                        </tr>
                ...
                <tfoot>
                    <tr>
                        <td colspan="{{$colspan}}">
                            SUBTOTAL&nbsp;&nbsp;&nbsp;$ {{number_format($suma_total,2)}}
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="{{$colspan}}">
                           TOTAL&nbsp;&nbsp;&nbsp;$ {{number_format($suma_total + $iva,2)}}
                        </td>
                    </tr>
                    ...
                    <td style="width: 50%">
                        <div class="footer-left" style="width: 70%">
                            ...
                            <div class="footer-contact">
                                <span>
                                   <b> TÉLEFONO</b> <br>
                                    {{$item->telefono_vendedor}}</span>
                            </div>
                            <div class="footer-contact">
                                <span>
                                    <b> CORREO ELECTRÓNICO</b> <br>
                                    {{$item->correo_vendedor}}</span>
                            </div>
                            <div class="footer-contact">
                                <span>
                                    <b> DIRECCIÓN</b> <br>
                                    {{$item->direccion_empresa_vendedor}}</span>
                            </div>
                        </div>
                    </td>

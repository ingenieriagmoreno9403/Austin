(function (window, document) {
    ...
    function initCostos() {
        ...
        function render() {
            ...
            tbody.innerHTML = items.map(function (it, idx) {
                ...
                return '<tr data-idx="' + idx + '">' +
                    ...
                    '<button type="button" class="cc-btn cc-btn-sm" data-sync-meses-api-costo' +
                    ' data-emp="' + escapeHtml(it.empresa) + '"' +
                    ' data-cod="' + escapeHtml(codigo) + '"' +
                    ' data-nom="' + escapeHtml(nombre || codigo) + '"' +
                    ' data-card="' + escapeHtml(it.card_code || '') + '"' +
                    ' title="Traer precios por mes (Ene–Dic) desde precios-mensuales">' +
            ...
            tbody.querySelectorAll('[data-sync-meses-api-costo]').forEach(function (btn) {
                btn.addEventListener('click', function () {
                    actualizarMesesDesdeApi(btn);
                });
            });
        }
        ...
        function actualizarMesesDesdeApi(btn) {
            var emp = btn.getAttribute('data-emp') || '';
            var cod = btn.getAttribute('data-cod') || '';
            var card = btn.getAttribute('data-card') || '';
            var nom = btn.getAttribute('data-nom') || cod;
            ...
            if (!card) {
                toast('error', 'Sin CardCode', 'Se necesita CardCode para consultar precios-mensuales.');
                return;
            }
            btn.disabled = true;
            var payloadBase = {
            ...
            fetch(url + '/actualizar-meses-desde-api', {
                method: 'POST',
                headers: apiJsonHeaders(),
                body: JSON.stringify(Object.assign({}, payloadBase, { confirmar: false }))
            }).then(function (r) {
                ...
                var mesesLbl = json.meses_label || ((json.meses || []).join(', ')) || '—';
                var msg = 'Producto: ' + (nom || cod) + '\n' +
                    'ItemCode: ' + cod + ' · CardCode: ' + card + '\n' +
                    'Año API: ' + (json.anio || payloadBase.anio) + '\n\n' +
                    'Meses encontrados: ' + mesesLbl + '\n\n' +
                    '¿Actualizar solo los precios por mes? El precio global no se modifica.';
                if (!window.confirm(msg)) return;

                return fetch(url + '/actualizar-meses-desde-api', {
                    method: 'POST',
                    headers: apiJsonHeaders(),
                    body: JSON.stringify(Object.assign({}, payloadBase, { confirmar: true }))
                }).then(function (r) {
                    ...
                    toast('success', 'Meses actualizados', j2.message ||
                        ((j2.guardados || 0) + ' mes(es) desde precios-mensuales'));
                    load(page);
                });
            }).catch(function (err) {
                toast('error', 'No se actualizó', err && err.message ? err.message : 'Error');

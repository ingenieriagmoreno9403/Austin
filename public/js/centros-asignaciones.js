(function (window, document) {
    ...
    function permBadges(perms) {
        return (perms || []).map(function (p) {
            return '<span class="cc-badge cc-badge-abierto">' + p + '</span>';
    ...
    function catalogoPermisos() {
        ...
        return list.length ? list : PERMS_FALLBACK;
    }
    ...
    function renderTablaFiltrada() {
        ...
        var rows = all.filter(function (r) {
            ...
            return matchFiltro(r.empresa, qEmp)
                ...
                && matchFiltro((r.centro_codigo || '') + ' ' + (r.centro_nombre || ''), qCc);
        });
    ...
    function renderTabla(rows, ciclo, total, filtrando) {
        ...
        if (!rows.length) {
            var empty = filtrando
                ...
                : 'Aún no hay asignaciones. Usa el botón Asignación para crear la primera.';
        ...
        rows.forEach(function (r) {
            ...
            html.push('<tr data-id="' + r.id + '">' +
                ...
                '<td>' + escapeHtml(r.centro_codigo) + ' — ' + escapeHtml(r.centro_nombre || '') + '</td>' +
    ...
    function openVer(row) {
        ...
        if (title) title.textContent = 'Ver · ' + row.centro_codigo;
        var cuentas = (p.cuentas || row.cuentas || []).map(function (c) {
            return '<li><span class="cc-cta-code">' + escapeHtml(c.codigo) + '</span> ' + escapeHtml(c.nombre || '') + '</li>';
        ...
        document.getElementById('asig-ver-body').innerHTML =
            ...
            '<div class="cc-context-item"><small>Centro</small><strong>' + escapeHtml(row.centro_codigo) + ' — ' + escapeHtml(row.centro_nombre || '') + '</strong></div>' +
    ...
    function openEditarNow(row) {
        ...
        if (title) title.textContent = 'Editar cuentas · ' + row.centro_codigo;
    ...
    function openPermisosNow(row) {
        ...
        if (title) title.textContent = 'Permisos · ' + row.centro_codigo;
    ...
    function eliminarAsignacion(row) {
        ...
        var text = esP && extras.length
            ...
            : 'Se quitará a ' + (row.usuario || 'este usuario') + ' de ' + (row.centro_codigo || 'este centro') + '.';
        var go = function () {
    ...
    CCAsig.initWizard = function (cfg) {
        ...
        function renderResumen() {
            ...
            tb.innerHTML = rows.map(function (a) {
                ...
                return '<tr class="' + cls.join(' ') + '" data-emp="' + escapeHtml(a.empresa) + '" data-key="' + escapeHtml(key) + '">' +
                    ...
                    '<td><div class="fw-semibold">' + escapeHtml(a.centro_codigo) + '</div>' +
                    '<div class="text-muted" style="font-size:.75rem">' + escapeHtml(a.centro_nombre || '') + '</div></td>' +
        ...
        function fillCentros(q) {
            ...
            if (keep && rows.some(function (c) { return String(c.codigo) === String(keep); })) {
        ...
        function recogerPayload() {
            ...
            if (!payload.centro_codigo) {
                if (window.Swal) Swal.fire({ icon: 'warning', title: 'Falta el centro', text: 'Elige un centro de costos.' });
                return null;
            }
            if (!ctas.length) {
                if (window.Swal) Swal.fire({ icon: 'warning', title: 'Sin cuentas', text: 'Selecciona al menos una cuenta.' });
